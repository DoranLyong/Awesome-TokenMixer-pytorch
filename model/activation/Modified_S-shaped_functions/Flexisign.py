...
class Flexisign(nn.Module):
    """ Flexiable Sign Function 
    """
    def __init__(self, alpha=1.0):
        super(Flexisign, self).__init__()
        self.alpha = torch.nn.Parameter(torch.tensor([alpha]), requires_grad=True)  # 가중치를 학습 가능한 매개변수로 설정

    def forward(self, x):        
        return x / (1 + torch.abs(self.alpha * x))
...
if __name__ == "__main__":
    ...
    # == Vis. == 
    plt.plot(x0.detach().numpy(), out0.detach().numpy(), c='skyblue', label=f'Flexisign alpha={0.5}')
    plt.plot(x0.detach().numpy(), x0_grad.detach().numpy(), c='skyblue', linestyle='--' ,label='Gradient')

    plt.plot(x1.detach().numpy(), out1.detach().numpy(), c='r', label=f'Flexisign  alpha={1.0} baseline')
    plt.plot(x1.detach().numpy(), x1_grad.detach().numpy(), c='r', linestyle='--' ,label='Gradient')

    plt.plot(x2.detach().numpy(), out2.detach().numpy(), c='b', label=f'Flexisign  alpha={2.0}')
    plt.plot(x2.detach().numpy(), x2_grad.detach().numpy(), c='b', linestyle='--' ,label='Gradient')

    plt.plot(x3.detach().numpy(), out3.detach().numpy(), c='g', label=f'Flexisign alpha={3.0}')
    plt.plot(x3.detach().numpy(), x3_grad.detach().numpy(), c='g', linestyle='--' ,label='Gradient')


    plt.title("Flexisign Activation Function")

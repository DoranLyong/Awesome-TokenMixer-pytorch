...
class ModifiedSoftsign(nn.Module):
    def __init__(self, alpha=1.0, scale=1.0):
        ...
        self.scale = torch.nn.Parameter(torch.tensor([scale]))  # 가중치를 학습 가능한 매개변수로 설정

    def forward(self, x):
        x = self.alpha * x
        return self.scale * (x / (1 + torch.abs(x)))
...
if __name__ == "__main__":
    ...
    # Activation instance 
    modified_softsign = ModifiedSoftsign(alpha=0.5, scale=1.0)  
    out0 = modified_softsign(x0)
    out0.backward(torch.ones_like(x0)) # out의 각 요소에 대해 역전파 수행
    x0_grad = x0.grad

    modified_softsign_a1 = ModifiedSoftsign(alpha=1.0, scale=1.0)  # baseline like; nn.Softsign() 
    out1 = modified_softsign_a1(x1)
    out1.backward(torch.ones_like(x1)) # out의 각 요소에 대해 역전파 수행
    x1_grad = x1.grad

    modified_softsign_a3 = ModifiedSoftsign(alpha=1.0, scale=0.5)
    out2 = modified_softsign_a3(x2)
    out2.backward(torch.ones_like(x2)) # out의 각 요소에 대해 역전파 수행
    x2_grad = x2.grad

    modified_softsign_b3 = ModifiedSoftsign(alpha=1.5, scale=2.0)
    ...
    # == Vis. == 
    plt.plot(x0.detach().numpy(), out0.detach().numpy(), c='skyblue', label=f'Softsign alpha={0.5}, scale={1.0}')
    ...
    plt.plot(x2.detach().numpy(), out2.detach().numpy(), c='b', label=f'Softsign  alpha={1.0}, scale={0.5}')
    plt.plot(x2.detach().numpy(), x2_grad.detach().numpy(), c='b', linestyle='--' ,label='Gradient')

    plt.plot(x3.detach().numpy(), out3.detach().numpy(), c='g', label=f'Softsign  alpha={1.5}, scale={2.0}')
